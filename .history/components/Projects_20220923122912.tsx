    ...
    return (
        <motion.div
            ...
            <div className='relative w-full flex overflow-x-scroll overflow-y-hidden snap-x snap-mandatory z-20  scrollbar-hide'>
                {skills.map(((skill, i) => (
                    <div key={skill._id} className='w-screen  flex-shrink-0 flex flex-col space-y-5 p-20md:p-44  h-screen bg-black/10 items-center justify-center snap-center text-center'>
                        <motion.img
                            initial={{ y: -300, opacity: 0 }}
                            transition={{ duration: 1.2 }}
                            whileInView={{ opacity: 1, y: 0 }}
                            viewport={{ once: true }}
                            src={urlFor(skill.image).url()} className='h-fit mt-32 md:mt-0 w-96 mx-auto' alt="" />
                        <div className='space-y-10  px-0 md:px-10 max-w-6xl'>
                            <h4 className='text-2xl md:text-4xl font-semibold text-center'>
                                <span className='underline decoration-[#f7ab0a]/50'>  Case Study {i + 1} of {projects.length}:</span> hrhrh
                            </h4>
