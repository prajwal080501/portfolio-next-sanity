        ...
        <motion.div
            ...
            <div className='relative w-full flex overflow-x-scroll overflow-y-hidden snap-x snap-mandatory z-20  scrollbar-hide'>
                {projects.map(((project, i) => (
                    <div key={project._id} className='w-screen  flex-shrink-0 flex flex-col space-y-5 p-20 md:p-44  h-screen bg-black/10 items-center justify-center snap-center text-center'>
                        <motion.img
                            initial={{ y: -300, opacity: 0 }}
                            transition={{ duration: 1.2 }}
                            whileInView={{ opacity: 1, y: 0 }}
                            viewport={{ once: true }}
                            src={urlFor(project.image).url()} className='h-52 md:h-[30vh] mt-52 md:mt-0 w-[60vh]  mx-auto' alt="" />
